...
import { calculate } from '@/lib/cotizador/calculators/vip'
...
export async function POST(req: NextRequest) {
  ...
  if (!sectionAllowed(user, 'vip')) return sectionForbidden()

  try {
    const body          = schema.parse(await req.json())
    const effectiveUser = applyComisionOverride(user, body.comisionOverride)
    const input = { ...body, quoteType: 'vip' as const }
    const tasas = await getEffectiveTasas('vip')
    const result = { '36': calculate(input, effectiveUser, 36, tasas), '48': calculate(input, effectiveUser, 48, tasas) }
    const folio = await saveQuote(user.id, 'vip', input, result)
