...
import { calculate } from '@/lib/cotizador/calculators/electrico'
...
export async function POST(req: NextRequest) {
  ...
  if (!sectionAllowed(user, 'electrico')) return sectionForbidden()

  try {
    const body          = schema.parse(await req.json())
    const effectiveUser = applyComisionOverride(user, body.comisionOverride)
    const input = { ...body, quoteType: 'electrico' as const }
    const tasas = await getEffectiveTasas('electrico')
    const result = { '24': calculate(input, effectiveUser, 24, tasas), '36': calculate(input, effectiveUser, 36, tasas), '48': calculate(input, effectiveUser, 48, tasas) }
